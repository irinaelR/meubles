...
create or replace view benefice as(
    SELECT  produit.IDTCS,produit.IDSTYLE, produit.IDCATEGORIE, produit.IDVOLUME,
        produit.NOMSTYLE, produit.NOMCATEGORIE, produit.NOMVOLUME, nbheures, prixvente , datepv , PRIXMAINDOEUVRE , prixvente-(nbheures*PRIXMAINDOEUVRE) as benefice
        from produit 
        join PRIXVENTEPRODUIT
        on PRIXVENTEPRODUIT.idproduit= produit.IDTCS
        join NBHORAIREPARSTYLE
        on NBHORAIREPARSTYLE.idstyle = produit.IDSTYLE
        join PRIXMAINDOEUVREPARTAILLE
        on PRIXMAINDOEUVREPARTAILLE.IDTAILLE = produit.IDVOLUME
...
--INSERT INTO EMPLOYE VALUES (DEFAULT, 'CHARPENTIER', '2022-12-01', 1, 1);
--INSERT INTO EMPLOYE VALUES (DEFAULT, 'CHARPENTIER', '2021-12-01', 2, 2);
...
--INSERT INTO PROFIL VALUES (DEFAULT, 1, 3, 1, 'SIMPLE OUVRIER');

CREATE OR REPLACE VIEW EXPERIENCEPAREMPLOYE AS (
    SELECT 
        TAUXHORAIRE,
        NOMEMPLOYE,
        EXTRACT(YEAR FROM CURRENT_TIMESTAMP) - EXTRACT(YEAR FROM DATEEMBAUCHE) + ANNEEEXPERIENCE AS EXPERIENCEACTUELLE
    FROM EMPLOYE
);

CREATE OR REPLACE VIEW PROFILACTUEL AS (
    SELECT
        NOMEMPLOYE, NOMPROFIL, TAUXHORAIRE*NIVEAU AS SALAIRE
        FROM EXPERIENCEPAREMPLOYE JOIN PROFIL ON 
